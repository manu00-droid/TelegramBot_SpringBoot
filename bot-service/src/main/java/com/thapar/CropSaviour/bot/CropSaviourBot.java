...
public class CropSaviourBot extends TelegramLongPollingBot {
    ...
    @Autowired
    private EnglishRemedyService englishRemedyService;

    @Autowired
    private HindiRemedyService hindiRemedyService;

    @Autowired
    private PunjabiRemedyService punjabiRemedyService;
    @Autowired
    private AudioService audioService;
    private final String IMAGE_DOWNLOAD_PATH = "/home/manav/Documents/Work/TelegramBot_SpringBoot/bot-service/src/main/java/com/thapar/CropSaviour/DownloadedImages/";
                    ...
                    if (!cropType.equalsIgnoreCase("weed")) {
                        ...
                        if (cropType.equalsIgnoreCase("rice")) {
                            if (language.equalsIgnoreCase("english"))
                                response = englishRemedyService.riceDiseaseRemedy(disease, chatId);
                            else if (language.equalsIgnoreCase("hindi"))
                                response = hindiRemedyService.riceDiseaseRemedy(disease, chatId);
                            else
                                response = punjabiRemedyService.riceDiseaseRemedy(disease, chatId);

                        } else if (cropType.equalsIgnoreCase("wheat")) {
                            if (language.equalsIgnoreCase("english"))
                                response = englishRemedyService.wheatDiseaseRemedy(disease, chatId);
                            else if (language.equalsIgnoreCase("hindi"))
                                response = hindiRemedyService.wheatDiseaseRemedy(disease, chatId);
                            else
                                response = punjabiRemedyService.wheatDiseaseRemedy(disease, chatId);
                        }
                        execute(response);
                        ...
                        String imgPath = classifierService.classifyDisease(IMAGE_DOWNLOAD_PATH + file, cropType);
                        System.out.println(file);
                        SendPhoto sendPhoto = new SendPhoto();
                        sendPhoto.setChatId(chatId);
                        java.io.File f = new File(IMAGE_DOWNLOAD_PATH + file);
    ...
    private void callBackQueryHandler(Update update) throws TelegramApiException {
        ...
        if (text.equalsIgnoreCase("L")) {
            ...
            if (data.substring(0, 2).equalsIgnoreCase("wh")) {
                data = "wheat";
            } else if (data.substring(0, 2).equalsIgnoreCase("we")) {
                data = "weed";
            } else {
                data = "rice";
            }
